...
def _case_name(fixed_dt: float) -> str:
    return f"dt_{fixed_dt:.0e}".replace("+", "").replace("-", "m")
...
def _plot_overlay(path: Path, histories: dict[str, list[Row]], *, key: str, ylabel: str, title: str) -> None:
    ...
    for index, (case, rows) in enumerate(sorted(histories.items())):
        ...
        values = np.asarray([float(row[key]) for row in rows], dtype=float)
        ax.plot(times, values, linewidth=1.3, color=colors[index % len(colors)], label=case.replace("dt_", r"$\Delta t=$"))
...
def _write_summary(path: Path, *, case_rows: list[Row], convergence: list[Row], outputs: dict[str, Path]) -> None:
    lines = [
        ...
        "## Cases",
        "",
        "| Case | Fixed dt | Status | Analysis wall (s) | Export wall (s) |",
        "| --- | ---: | --- | ---: | ---: |",
    ]
    for row in case_rows:
        ...
        lines.append(
            f"| `{row['case']}` | {float(row['fixed_dt']):.1e} | {row.get('status', '')} | {analysis_text} | {export_text} |"
        )
...
def run_convergence(
    ...
    *,
    ...
    for fixed_dt in dt_values:
        ...
        try:
            row = _run_case(cfg, out_dir, abaqus_command=abaqus_command, convert_odb=convert_odb)
    ...
    if histories:
        ...
        _write_csv(outputs["convergence"], convergence)
    _write_summary(outputs["summary"], case_rows=case_rows, convergence=convergence, outputs=outputs)
    return outputs
