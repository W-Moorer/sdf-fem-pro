...
def write_calculix_input(model: DropModel, path: Path) -> None:
    ...
    lines.extend(
        [
            ...
            f"*dynamic, direct, alpha={model.hht_alpha:.12g}",
            ...
            "u",
            "*node print, nset=nfloor, totals=only, global=yes, frequency=1",
            "rf",
...
def write_markdown(
    ...
    *,
    ...
    lines = [
        ...
        "",
        "This validation uses CalculiX/ccx as an external open-source transient dynamic contact reference. The generated models are vertical elastic drop-impact cases against a fixed rigid plane. SFC uses the same TET4 meshes, material constants, gravity, initial velocity, direct HHT-alpha integration parameters, and rigid-plane location as each CalculiX input.",
        "",
        "## Reproduce",
        "",
        "```bash",
...
def run_comparison(out_dir: Path, *, quick: bool) -> dict[str, Path]:
    ...
    _write_csv(
        ...
        [
            ...
            {"key": "calculix_dynamic_keyword", "value": "*DYNAMIC,DIRECT,ALPHA=-0.05"},
