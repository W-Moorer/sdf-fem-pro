...
| CalculiX C3D8 contactenergy replay | Official CalculiX `contactenergy.inp` solves a static C3D8 surface-to-surface contact-energy reference. SFC replays the final CalculiX deformed geometry with `dynamic_surface_sdf` on triangulated current C3D8 boundary faces and compares normal force plus total contact spring energy. | Implemented in `validation/run_calculix_contactenergy_replay.py`; locked under `paper/numerical_experiments/calculix_contactenergy_c3d8_replay`; covered by `tests/test_calculix_contactenergy_replay.py` and locked paper tests when the local CalculiX test data are available. | C3D8 static contact-law/energy replay only. It demonstrates that dynamic SDF is not TET4-bound, but it is not a C3D8 mechanics implementation or TET4 trajectory-equivalence claim. |
