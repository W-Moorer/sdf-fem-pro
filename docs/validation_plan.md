...
| Geometric nonlinear VTK diagnostic | Total-Lagrangian St. Venant-Kirchhoff TET4 block/plane contact writes continuous ParaView VTK frames with displacement, velocity, Green-Lagrange strain, Cauchy stress, and von Mises stress fields. | Implemented in `validation/run_geometric_nonlinear_vtk.py`; latest output documented in `docs/geometric_nonlinear_vtk_handoff.md`. | Validation-only explicit diagnostic exporter; not yet the production nonlinear implicit SFC solver and not a full CalculiX-equivalent nonlinear contact implementation. |
