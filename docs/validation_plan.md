...
The core FEM assembly now goes through an element-backend registry. Registered
mechanics backends currently include C3D4/TET4 and HEX8/C3D8 linear elements.
C3D10 mechanics is not registered yet.
...
contains linear static, linear Newmark dynamic, nonlinear static Newton, and
nonlinear HHT/Newmark adapters around the existing assembly/integration kernels.
The current contact layer contains a penalty normal-contact adapter around the
existing dynamic-SDF constraints and Gauss-Newton contact stiffness path.
...
| Case | Expected quantities | Current status | Limitations |
| --- | --- | --- | --- |
| SfePy two-body contact replay | SfePy solves its built-in two-body nonlinear penalty contact example. SFC rebuilds a true current-space `DynamicNarrowBandSDF` field from the final master surface and reports interpolated field gap sign, gap magnitude, active contact count, field update metadata, and agreement with SfePy exported contact gaps. | Implemented in `validation/run_external_contact_solver_comparison.py`; covered by `tests/test_external_contact_solver_comparison.py` when SfePy is available and by `tests/test_open_source_contact_benchmark_plan.py`. | This validates true-field contact state/gap replay against an external contact solve; it is not a full SFC nonlinear contact equilibrium comparison. |
| FuzzyContact VTU field reference | The public Mendeley FuzzyContact dataset provides optional VTU fields with displacement, strain, and stress data. SFC records dataset metadata and, when local VTU files are provided, parses 3D fields and renders a surface preview for visual audit. | Implemented in `validation/run_fuzzycontact_vtu_reference.py`; covered by `tests/test_fuzzycontact_vtu_reference.py`. | Optional external field-cloud reference only. The large dataset is not downloaded by default and is not a core dependency. |
| External contact claim gates | Contact state agreement and gap-scale checks must have backing CSV evidence before paper text can cite the external contact solver comparison. | Implemented in `external_contact_solver_claims.csv`. | SfePy gap is a contact-term surface average; SFC gap is a centroid true-field interpolation replay. |
...
| Clean-room CalculiX-aligned mechanics backend | Provides a swappable mechanics/contact interface with consistent TET4 mass, StVK internal force, material tangent, geometric tangent, HHT/Newmark Newton stepping, contact samples, and optional clean-room CalculiX-style multi-criterion Newton acceptance diagnostics using `ram/qam/cam/uam`, contact element change, and contact-energy stabilization proxies. | Implemented in `src/sfc/fem/calculix_aligned.py` and covered by `tests/test_calculix_aligned_backend.py`. | Clean-room implementation using public formulas; it is not copied CalculiX source and does not reproduce every CalculiX solver option. The multi-criterion policy is optional and must be validated before replacing the default relative-correction stop. |
| CalculiX C3D4 face-to-face validation contact mode | Adds validation-only C3D4 F2F contact modes with seven slave-face integration springs per linear triangular TET4 face in strict CalculiX-aligned modes, current face-area weighting, hard linear pressure-overclosure, stored master projection/normal clearance, persistent spring bookkeeping, active cutback/retry diagnostics, CalculiX `CNUM`-equivalent generated contact-element counting, and deformable-deformable master/slave `J^T lambda` / `J^T J` assembly. The same enforcement can be driven by an analytic plane query or the SFC dynamic-SDF query. | Implemented in `validation/calculix_f2f_contact.py`; wired into `validation/run_geometric_nonlinear_contact_validation.py` as `--contact-mode persistent_calculix_c3d4_f2f`, `--contact-mode calculix_c3d4_f2f`, `--contact-mode persistent_dynamic_sdf_calculix_f2f`, and `--contact-mode dynamic_sdf_calculix_f2f`; covered by `tests/test_calculix_f2f_contact.py`. | This is a clean-room validation alignment layer. It preserves duplicate CalculiX contact rows through a `contact_element_index` and now reproduces the CalculiX-displacement replay `CNUM` lifecycle for the checked block-plane case. It narrows the lifecycle/output-definition gap but still does not copy CalculiX source or reproduce every private/source-level CalculiX branch. |
...
| Deforming-master dynamic-SDF reference | Compares spatial-hash dynamic SDF against brute-force all-face projection on prescribed stretched, sheared, and bent current master surfaces, including assembled penalty-force checks and a frozen reference-surface SDF baseline. | Implemented in `validation/run_deforming_master_sdf_reference.py`; latest output documented in `docs/deforming_master_sdf_reference_handoff.md`; covered by `tests/test_deforming_master_sdf_reference.py`. | Prescribed master deformation only; not a coupled two-body nonlinear solve or external solver contact validation. The frozen baseline is validation-only evidence showing the error caused by not updating the SDF with current master coordinates. |
| CalculiX deformable dynamic-SDF replay | CalculiX solves a two-deformable-block normal-compression contact state; SFC replays the final deformed coordinates with current-surface `dynamic_surface_sdf` queries and compares contact sign plus minimum gap against CalculiX `CDIS` over r1/r2/r3 mesh resolutions. | Implemented in `validation/run_calculix_deformable_sdf_contact_validation.py`; latest paper output documented in `docs/calculix_deformable_sdf_contact_validation.md`; covered by `tests/test_calculix_deformable_sdf_contact_validation.py` when CalculiX is available. | Lateral DOFs are fixed to isolate normal contact-gap evaluation. This validates external deformed-state SDF replay, not full native SFC/CalculiX trajectory equivalence, and no convergence order is claimed. |
| CalculiX C3D8 contactenergy replay | Official CalculiX `contactenergy.inp` solves a static C3D8 surface-to-surface contact-energy reference. SFC runs the companion static solve through the registered C3D8 stiffness backend, then replays the final CalculiX deformed geometry with `dynamic_surface_sdf` on triangulated current C3D8 boundary faces and compares normal force plus total contact spring energy. | Implemented in `validation/run_calculix_contactenergy_replay.py`; locked under `paper/numerical_experiments/calculix_contactenergy_c3d8_replay`; covered by `tests/test_calculix_contactenergy_replay.py` and locked paper tests when the local CalculiX test data are available. | C3D8 static contact-law/energy replay and linear C3D8 companion solve only. It demonstrates that dynamic SDF is not TET4-bound, but it is not yet a nonlinear C3D8 trajectory-equivalence claim. |
| CalculiX C3D8 dynamic contact trajectory replay | Generated C3D8 block-plane and block-block transient contact cases are solved by CalculiX. SFC replays every exported deformed C3D8 configuration with current-surface `dynamic_surface_sdf` on triangulated C3D8 boundary quads and compares `CDIS`, fixed-support `RF`, `CELS`, active-contact count, center height, and final stress-cloud diagnostics. | Implemented in `validation/run_c3d8_contact_trajectory_validation.py`; locked quick output is stored under `paper/numerical_experiments/c3d8_contact_trajectory_validation`; covered by `tests/test_c3d8_contact_trajectory_validation.py`. | This is external trajectory replay evidence, not native SFC nonlinear C3D8 dynamics equivalence. CalculiX supplies the displacement trajectory; SFC validates the current-surface dynamic SDF contact geometry and force/energy replay on C3D8 surfaces. |
| Phase-9 full contact validation matrix | Organizes the requested static/dynamic, linear/geometric-nonlinear, C3D8 contact evidence and applies hard claim gates: no native SFC result means no trajectory-equivalence claim, no CalculiX comparison means no external-correctness claim, and no timing evidence means no efficiency claim. Outputs CSV, Markdown, PNG/PDF plots, ParaView VTK stress/strain clouds, a C3D4/C3D8 side-by-side table, per-case SFC/CalculiX wall-time rows, and a warped/non-planar C3D8 external replay diagnostic. | Implemented in `validation/run_phase9_full_contact_validation.py`; locked quick output is stored under `paper/numerical_experiments/phase9_full_contact_validation`; covered by `tests/test_phase9_full_contact_validation.py` and locked paper tests. The native backend now includes clean-room C3D8 linear dynamic Newmark contact for block-plane and block-block, StVK 2x2x2 Gauss integration, C3D8 mass, nonlinear static Newton comparison, and C3D8 block-plane/block-block nonlinear HHT trajectory runners. | Supported rows are claim-gated per output CSV. Native C3D8 linear dynamic and nonlinear dynamic block-plane/block-block contact are compared against CalculiX trajectories. Native C3D8 nonlinear static contact is compared against the CalculiX `contactenergy` reference. The warped/non-planar C3D8 row is an external comparison diagnostic, but current RF/CELS/CDIS metrics do not pass the external-correctness gate because CalculiX generated-contact-element output definitions differ from the SFC quadrature replay. For CalculiX linear dynamic rows with empty CDIS detail output, the gap metric is a CalculiX-displacement current-surface replay gap. C3D10 is explicitly out of scope. |
...
| Geometric nonlinear contact external validation | Compares gravity-driven block-plane geometric nonlinear contact against CalculiX for activation time, max penetration, normal reaction, contact energy, z-center history, rebound height, and contact-zone stress. Also writes r1/r2/r3 mesh trends, dt/dt/2/dt/4 time-step trends, contact-law replay diagnostics, increment-acceptance diagnostics, HHT residual/tangent trajectory diagnostics, contact lifecycle/output diagnostics, mechanics/increment-acceptance diagnostics, one-step CalculiX-state mechanics diagnostics, HHT/Newmark state-definition and `.dat` precision diagnostics, per-contact-element CDIS/CSTR/CELS clearance/lifecycle audit, and paired ParaView VTK stress clouds. | Implemented in `validation/run_geometric_nonlinear_contact_validation.py`; default SFC mode is now `persistent_calculix_c3d4_f2f` with `--cutback-policy calculix_direct` to match the generated CalculiX `*DYNAMIC, DIRECT` input. `--cutback-policy active_retry` and older non-persistent/three-point quadrature modes are retained as variants. The generated CalculiX input now requests per-contact CDIS/CSTR/CELS plus totals-only CNUM. HHT residual/tangent diagnostics are documented in `docs/calculix_hht_residual_tangent_diagnostics.md`; contact spring lifecycle, clearance update, and `CNUM/CELS/RF` output definitions are documented in `docs/calculix_contact_lifecycle_output_definitions.md`; mechanics/increment acceptance is documented in `docs/calculix_mechanics_increment_acceptance_diagnostics.md`; source-alignment migration scope is documented in `docs/calculix_source_alignment_migration_plan.md`. Latest output documented in `docs/geometric_nonlinear_contact_validation_handoff.md`. | Scoped to a regular block-plane benchmark. The strict persistent mode aligns C3D4 face integration, hard-linear pressure law, generated spring diagnostics, CalculiX `CNUM`-equivalent counting, direct fixed-increment behavior, and the HHT residual/tangent sign convention. The CalculiX-aligned backend stores accepted previous history in the `fextini - fini` sign, with contact spring reactions included through internal spring force history. The one-step CSV starts SFC from CalculiX displacement states and decomposes SFC internal force, contact force, mass term, HHT residual, tangent norms, and Newmark update error; the HHT state CSV records CalculiX source-level beta/gamma, prediction/update/residual conventions, initial acceleration regularization, mass scale, and displacement-print precision amplification. CalculiX full global internal force/tangent remain unavailable unless explicitly exported by a future reference path. It remains a clean-room approximation rather than copied CalculiX internals; the diagnostics explain remaining RF/CELS/max-penetration trajectory differences, not exact source-level equivalence. |
| Dynamic-SDF full trajectory external validation | Runs the native SFC `persistent_dynamic_sdf_calculix_f2f` trajectory against CalculiX for a 1 second block-drop benchmark, comparing contact activation, center-of-mass trajectory, penetration, normal force, contact energy, CNUM lifecycle, rebound height, and contact-zone stress. | Implemented through `validation/run_geometric_nonlinear_contact_validation.py --contact-mode persistent_dynamic_sdf_calculix_f2f --curve-only`; latest 1s output documented in `docs/dynamic_sdf_full_trajectory_calculix_validation.md`; quick external regression covered by `tests/test_geometric_nonlinear_contact_validation.py` when CalculiX is available. | This is the paper-facing full trajectory validation for the SDF contact path, but it remains scoped to block-plane frictionless normal contact. It does not validate sphere-like drop or arbitrary contact geometries. |
| Official CalculiX contact example line | Catalogs and optionally runs the official CalculiX examples selected for the next paper evidence line: `contactenergy.inp`, `scheibe2f2f.inp.gz`, `ball.inp.gz`, and `contact1/contact3/contact6`. Reports element types, contact-pair type, step type, pressure-overclosure law, CalculiX run status, DAT/FRD availability, and claim gates. | Implemented in `validation/run_calculix_official_contact_examples.py`; outputs are intended for `paper/numerical_experiments/calculix_official_contact_examples`; covered by `tests/test_calculix_official_contact_examples.py`. | Only `contactenergy.inp` is currently allowed as a direct paper reference. `contact1/contact3/contact6` are law-alignment references. `scheibe2f2f` and `ball` are official engineering candidates that still require native SFC metric extraction before full-field or full-trajectory claims. |
| Official CalculiX contact-law + deforming-SDF replay | Instruments `contactenergy.inp` and `contact1/contact3/contact6` with contact-output requests, lets CalculiX solve the official C3D8 contact models, then replays the final deformed state with SFC current-surface dynamic SDF. Outputs per-contact-point `CDIS`/gap, `CSTR`/pressure, `CELS`/energy diagnostics, claim gates, CSV, Markdown, and PNG/PDF plots. | Implemented in `validation/run_calculix_official_law_sdf_replay.py`; locked output is stored under `paper/numerical_experiments/calculix_official_law_sdf_replay`; covered by `tests/test_calculix_official_law_sdf_replay.py` and locked paper tests. | This validates deformed-state gap and pressure-law replay, not native SFC trajectory equivalence. Node-to-surface `CELS` uses CalculiX internal spring area, so total energy equivalence is claimed only for the official `contactenergy` surface-to-surface row. |
| Official `scheibe2f2f` FRD replay and `ball` native trajectory probe | Parses CalculiX FRD `DISP` and `CONTACT` blocks from the official `scheibe2f2f.inp.gz`, reconstructs the deformed C3D8 master surface, and replays SFC current-surface SDF gap/pressure at each reported `COPEN/CPRESS` contact node. The same runner instruments the official `ball.inp.gz`, runs a native SFC C3D8 dynamic drop against the rigid floor plane, and compares center trajectory/contact diagnostics against CalculiX DAT output where available. | Implemented in `validation/run_calculix_scheibe_ball_sdf_validation.py`; locked output is stored under `paper/numerical_experiments/calculix_scheibe_ball_sdf_validation`; covered by `tests/test_calculix_scheibe_ball_sdf_validation.py` and locked paper tests. | `scheibe2f2f` supports the deformed-state C3D8 SDF gap/pressure replay claim. It is not a native nonlinear-static SFC trajectory claim. In the locked 0.13 s `ball` run, SFC reaches contact but CalculiX fails near first contact and prints no active-contact rows after 0.116 s; therefore the ball row is retained as a native-runner probe and failure-boundary diagnostic, not as full dynamic contact trajectory equivalence evidence. |
