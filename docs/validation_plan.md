...
| CalculiX deformable dynamic-SDF replay | CalculiX solves a two-deformable-block normal-compression contact state; SFC replays the final deformed coordinates with current-surface `dynamic_surface_sdf` queries and compares contact sign plus minimum gap against CalculiX `CDIS`. | Implemented in `validation/run_calculix_deformable_sdf_contact_validation.py`; latest quick output documented in `docs/calculix_deformable_sdf_contact_validation.md`; covered by `tests/test_calculix_deformable_sdf_contact_validation.py` when CalculiX is available. | Lateral DOFs are fixed to isolate normal contact-gap evaluation. This validates external deformed-state SDF replay, not full native SFC/CalculiX trajectory equivalence. |
