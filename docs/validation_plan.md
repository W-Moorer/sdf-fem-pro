...
| Gentle CalculiX contact reference | A low-speed `block_drop` case can be run for 1 second as a drop from rest under gravity with configurable contact stiffness to avoid the strong-impact convergence failures seen in the earlier diagnostic. | Supported by options in `validation/run_calculix_drop_impact_comparison.py` (`--case`, `--initial-velocity-z`, `--gravity`, `--contact-stiffness`) and analyzed in `docs/calculix_alignment_analysis.md`; the latest gentle run passes the rebound-height physicality gate. | This validates only the gentle block/plane reference. The strong 1-second impact diagnostic remains unsupported because it either times out or rebounds above the starting center-of-mass height. |
