...
| Gentle CalculiX contact reference | A low-speed `block_drop` case can be run for 3 seconds with no gravity and configurable contact stiffness to avoid the strong-impact convergence failures seen in the earlier diagnostic. | Supported by options in `validation/run_calculix_drop_impact_comparison.py` (`--case`, `--initial-velocity-z`, `--gravity`, `--contact-stiffness`) and analyzed in `docs/calculix_alignment_analysis.md`. | This is a candidate external reference; it should not support a paper claim unless CalculiX completes and the generated claim gate is supported. |
