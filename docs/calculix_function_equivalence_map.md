...
- full `gencontelem_f2f` lifecycle branch coverage;
...
1. Contact spring lifecycle: generated, persisted, released, and reactivated
   states with explicit per-spring state transitions.
2. Increment acceptance: residual/correction/contact-change criteria as
   standalone functions with test vectors.
3. Contact stabilization and `kscale`: explicit pressure-stiffness scaling
   state and restoration behavior.
4. Output definitions: CNUM/CELS/RF calculations from the same generated spring
