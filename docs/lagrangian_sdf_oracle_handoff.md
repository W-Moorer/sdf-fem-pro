...
该 runner 的 current-surface projection 只用于内部回归检查，不作为论文 baseline、主文对比路径或效率 claim 依据。论文级对比应改为 SFC final-aim oracle 与 CalculiX native contact 的结果/效率对比。
...
- `lagrangian_patch_oracle` 在复杂变形曲面上保持机器精度级几何一致性。
- 该证据只用于工程开发的几何回归。
- 论文最终结果/效率对比应以 CalculiX native contact 为外部参考，不再展示 brute-force projection 对比。
