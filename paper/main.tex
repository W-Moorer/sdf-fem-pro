...
\graphicspath{{../results/paper_final/figures/}{../results/phase7/}{../results/external_fem/}{../results/phase8/}{../results/external_contact_solver/}{../results/external_dynamic_contact/}{results/paper_final/figures/}{results/phase7/}{results/external_fem/}{results/phase8/}{results/external_contact_solver/}{results/external_dynamic_contact/}}
...
We implement the method in a standalone FEM-SDF prototype with internally assembled TET4 stiffness, mass, body-force, constraint, contact, and time-integration components. The core solver does not depend on Abaqus-generated matrices, force files, \texttt{.odb} files, or spreadsheet outputs. Experiments verify the linear TET4 implementation against analytic small-strain stress, strain, and energy references; compare stiffness, displacement, strain, stress, and von Mises fields against an equivalent scikit-fem model; replay SfePy contact states with SFC gap queries; compare against a SfePy transient-elastodynamic reference with mass and Newmark integration; compare contact forces against brute-force closest-point projection; evaluate a frozen material-space SDF baseline under deformation; and report repeated timing results. In the final non-quick experiments, the dynamic FEM-induced surface-distance pipeline achieves speedups from \(3.58\times\) to \(57.07\times\) over brute-force all-triangle projection for the tested surface resolutions.
...
\subsection{Engineering-style validation cases}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.96\linewidth]{phase8_cantilever_external_stress_3d.pdf}
  \caption{Phase-8 engineering cantilever comparison against scikit-fem. The panels show nodal-averaged interpolated SFC stress, scikit-fem stress, and absolute stress error on the 3D boundary surface.}
  \label{fig:phase8-cantilever}
\end{figure}

\begin{figure}[t]
  \centering
  \includegraphics[width=0.82\linewidth]{phase8_rigid_indenter_stress_3d.pdf}
  \caption{Rigid flat indenter pressing an elastic block. The block color shows nodal-averaged interpolated von Mises stress under the prescribed penalty contact load; the rigid indenter is drawn as a dark plotting primitive above its contact underside.}
  \label{fig:phase8-indenter}
\end{figure}

\begin{figure}[t]
  \centering
  \includegraphics[width=0.82\linewidth]{phase8_deformable_deformable_stress_3d.pdf}
  \caption{Deformable-deformable block contact stress visualization. The Phase-8 CSV outputs report gap, penetration, contact force, contact energy, and action-reaction imbalance for the prescribed approach sequence.}
  \label{fig:phase8-deformable}
\end{figure}
...
\subsection{External transient elastodynamic and contact-snapshot reference}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.78\linewidth]{external_dynamic_contact_min_gap.pdf}
  \caption{External transient/contact reference. The SfePy transient elastodynamic run provides the 3-second Newmark/mass reference, while SfePy contact snapshots and SFC replay compare contact activation and gap scale along the same path.}
  \label{fig:external-dynamic-contact}
\end{figure}
