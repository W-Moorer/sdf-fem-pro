...
The final paper-scale performance and material-space baseline results are taken from the Phase-6 non-quick experiment package under \path{results/paper_final}. Physical-comparison evidence is generated by the Phase-7 package and the locked paper numerical experiments under \path{paper/numerical_experiments}. We also maintain a conservative official CalculiX example line: \texttt{contactenergy.inp} is the direct C3D8 static contact reference; \texttt{contact1/contact3/contact6} now provide official deformed-state SDF gap and pressure-overclosure replay diagnostics against CalculiX \texttt{CDIS}/\texttt{CSTR}; and \texttt{scheibe2f2f.inp.gz} plus \texttt{ball.inp.gz} are recorded as future engineering candidates until native SFC metric-level comparisons are implemented. The recorded Phase-6 runtime metadata is Python 3.13.5, NumPy 2.1.3, SciPy 1.15.3, Matplotlib 3.10.0, Windows 11 AMD64, and 24 logical CPUs.
