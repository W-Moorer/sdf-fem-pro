...
We implement the method in a standalone FEM-SDF prototype with internally assembled TET4 stiffness, mass, body-force, constraint, contact, and time-integration components. The core solver does not depend on Abaqus-generated matrices, force files, `.odb` files, or spreadsheet outputs. Experiments verify the linear TET4 implementation against analytic small-strain stress, strain, and energy references; compare stiffness, displacement, strain, stress, and von Mises fields against an equivalent scikit-fem model; replay SfePy contact states with SFC gap queries; compare against a SfePy transient-elastodynamic reference with mass and Newmark integration; compare contact forces against brute-force closest-point projection; evaluate a frozen material-space SDF baseline under deformation; and report repeated timing results. In the final non-quick experiments, the dynamic FEM-induced surface-distance pipeline achieves speedups from \(3.58\times\) to \(57.07\times\) over brute-force all-triangle projection for the tested surface resolutions.
...
### 5.6 External transient elastodynamic and contact-snapshot reference

To add time-history evidence without relying on a rigid-body contact engine, we also run a generated SfePy transient elastodynamic reference over \(T=3\,\mathrm{s}\). This external run includes a mass term and SfePy Newmark integration. Along the same 3-second approach path, SfePy's two-body penalty contact example is solved at sampled times, and each contact snapshot is replayed with the SFC current-surface distance query. This separates two reference roles: SfePy checks the transient FEM mass/Newmark setup, while SfePy contact snapshots provide independent contact-state evidence. We do not claim this is a single coupled SfePy transient nonlinear contact solve.

Paper figure:

- `results/external_dynamic_contact/external_dynamic_contact_min_gap.pdf`

### 5.7 Dynamic surface-distance reference comparison
...
### 5.8 Contact force-displacement reference
...
### 5.9 Material-space SDF baseline
...
### 5.10 Contact time-history diagnostics
...
The physical comparison evidence is also bounded. The stress/strain check is an analytic affine patch test, and the external open-source FEM comparison uses scikit-fem on matching linear TET4 cantilever models. The external contact-solver comparison uses SfePy's two-body penalty contact example and replays the final deformed contact state with SFC gap queries. The transient external evidence uses a separate SfePy elastodynamic Newmark run and SfePy contact snapshots along the same path; it is not a single coupled SfePy transient contact solve. These results verify consistency of the implemented linear formulation, reported stress quantities, time-integration setup, and current-surface contact gap evaluation for the tested cases, but they are not yet comparisons against Abaqus, CalculiX, FEniCS, experimental measurements, nonlinear FEM, or a production contact code.
